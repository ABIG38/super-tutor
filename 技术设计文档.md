...
用户提问
    ...
    │     ChromaDB + BAAI/bge-m3（云端 SiliconFlow API）
...
vector_rank[chunk] + bm25_rank[chunk] → 按 RRF 降序排列 → 去重
  ...
  取 RRF Top 15 → BAAI/bge-reranker-v2-m3（云端 SiliconFlow API）
...
- 模型：`BAAI/bge-reranker-v2-m3`，通过 SiliconFlow 云端 API 调用（非本地加载），免去 GPU 依赖
...
| 文件 | 职责 |
|------|------|
| `vector_store.py` | `VectorStore` — ChromaDB 封装：add_chunks / search / count / delete_all / get_source_files / delete_by_source。使用 `BAAI/bge-m3` 云端 embedding API（SiliconFlow） |
...
```
> 若 chunk 有所属课程，标注为 `[来源: {filename}（课程: {course}）]`；否则为 `[来源: {filename}]`。
...
| Embedding | SiliconFlow API (BAAI/bge-m3) | | 云端 API 调用，免本地 GPU 依赖 |
| 关键词检索 | rank-bm25 + jieba | | BM25 + 中文分词 |
| Reranker | SiliconFlow API (BAAI/bge-reranker-v2-m3) | | 云端 API 调用，免本地 GPU 依赖 |
...
| 打包 | PyInstaller | | 单 exe 发布；Embedding/Reranker 改为云端 API，无需打包模型文件 |
...
│   ├── retrieval/
│   │   ├── __init__.py
│   │   ├── vector_store.py        # VectorStore (ChromaDB + bge-m3 云端 API)
...
├── knowledge_base/
│   ├── models/                    # Embedding / Reranker 模型缓存（云端 API 时可选）
│   ├── raw/                       # 用户上传的原始文档（可选保留）
│   └── index/
...
| Embedding | `BAAI/bge-m3`（云端 SiliconFlow API） | 中文语义理解强；云端调用免本地 GPU 依赖；专业术语召回率高 |
...
| 并发控制 | `threading.Lock()` + QThread | 单用户场景，普通互斥锁足够；QThread 保证主 UI 不卡。注意：Python GIL 下 Embedding 云端 API 调用为 I/O 密集型，QThread 足够；无需升级到多进程 |
...
| 打包 | PyInstaller + 云端 API | 单文件 exe；Embedding/Reranker 改为云端 API 后不再需要打包离线模型，打包体积大幅减小 |
...
class Settings(BaseSettings):
    # LLM
    llm_api_key: str = "MISSING_KEY"          # 必填，缺失启动报错
    llm_api_base: str = "https://api.deepseek.com/v1"
    llm_model: str = "deepseek-chat"

    # Embedding（云端 SiliconFlow API）
    ...
    # Storage
    storage_root: str = "knowledge_base"       # 数据存储根目录，用户可自定义
...
| ⑱ | Embedding API Key 缺失 | 启动时检测 `embedding_api_key` 为空 | 弹窗引导用户在设置中配置 SiliconFlow API Key。若用户跳过 → 检索降级为仅 BM25 关键词检索，并向用户提示「语义检索不可用」 | orchestrator.py + frontend |
| ⑲ | Embedding API 调用失败 | ChromaDB 查询时抛 `APIConnectionError` / 返回空结果 | catch → 日志记录错误，UI 提示「语义检索服务异常」，降级为仅 BM25 关键词检索，不阻塞问答流程 | vector_store.py |
...
桌面应用当前 Embedding / Reranker 改为云端 API 调用，不再需要打包离线模型文件。
打包时仅需确保 Python 依赖完整即可。`super-tutor.spec` 配置详见项目根目录。
