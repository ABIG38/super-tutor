...
│                     用户界面 (PySide6 桌面)                          │
│  ┌──────────┐  ┌────────────┐  ┌──────────┐  ┌───────────────┐     │
│  │ 知识库列表 │  │  问答对话页  │  │  复习规划  │  │ 学习进度面板   │     │
│  │ (文档管理) │  │ (流式渲染)  │  │ (计划展示) │  │ (打勾标记进度) │     │
│  └──────────┘  └────────────┘  └──────────┘  └───────┬───────┘     │
└────────────────────────────────────────────────────────┼────────────┘
                           │                              │ 反馈
                           ▼                              │
...
│                           基础设施层                                  │
│  ┌──────────┐  ┌──────────────┐  ┌─────────────────────┐ ┌───────┐ │
│  │ ChromaDB │  │ 本地文件系统   │  │ LLM API（OpenAI 兼容）│ │SQLite │ │
│  │ (向量库)  │  │ (原始文档)    │  │ (溯源回答生成)       │ │(进度) │ │
│  └──────────┘  └──────────────┘  └─────────────────────┘ └───────┘ │
└─────────────────────────────────────────────────────────────────────┘
```
...
- **UI 层**：PySide6 原生桌面窗口，四区布局（知识库列表 + 问答 Tab + 规划 Tab + 学习进度面板）。每页顶部有课程选择器下拉框（从 courses.json 加载），当前选定课程作为检索/规划/追踪的上下文范围。用户可新建/重命名/删除课程
...
| **M6** | 桌面 UI 完善：四区布局（含学习进度面板）、流式渲染、QThread 异步、状态栏、异常处理 | 1 周 | 完整可用 |
